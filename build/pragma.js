...
eval("\n    var riot = __webpack_require__(/*! riot */ \"./node_modules/riot/riot.js\")\n    riot.tag2('tree', '<virtual each=\"{child, index in children}\"> <virtual if=\"{depth === 0}\"> <fieldset> <legend>{child.name}</legend> <virtual if=\"{child.children}\"> <tree children=\"{child.children}\" depth=\"{depth + 1}\"></tree> </virtual> <virtual if=\"{!child.children}\"> <div> <input name=\"{child.path}\" riot-value=\"{child.default}\"> </div> </virtual> </fieldset> </virtual> <virtual if=\"{depth > 0}\"> <p> <label> <span>{\\'&nbsp; \\'.repeat(depth - 1)} {child.name || child.path}</span> <virtual if=\"{!child.children}\"> <input name=\"{child.path}\" riot-value=\"{child.default}\" disabled=\"{!!child.derivation}\"> </virtual> </label> </p> <virtual if=\"{child.children}\"> <tree children=\"{child.children}\" depth=\"{depth + 1}\"></tree> </virtual> </virtual> </virtual>', '', '', function(opts) {\nthis.children = this.opts.children || [];\nthis.depth = parseInt(this.opts.depth) || 0;\n});\n\n    \n  \n\n//# sourceURL=webpack://pragma/./src/tags/tree.tag?");
